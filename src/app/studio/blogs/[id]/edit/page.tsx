...
export default function BlogEditorPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        async function fetchData() {
            ...
            if (blogRes.success && blogRes.blog) {
                ...
                setSelectedTags(b.hashtags || []);
                setCoverImage(b.heroImage || '');
    ...
    const handleSave = async (status: 'DRAFT' | 'PUBLISHED') => {
        ...
        setIsSaving(false);
        if (res.success) {
            toast.success(`Blog ${status.toLowerCase()} successfully updated!`);
        ...
        <div className="p-6 max-w-7xl mx-auto space-y-6 animate-fadeIn">
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-textPrimary">Edit Publication</h1>
                        <p className="text-xs text-textSecondary">Update your engineering piece.</p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button onClick={() => handleSave('PUBLISHED')} disabled={isSaving} className="px-4 py-2 bg-slate-900 text-white font-bold rounded-xl text-sm flex items-center gap-2 hover:bg-gold-500 hover:text-slate-900 transition-colors shadow-sm">
                        <Send className="w-4 h-4" /> Update Now
                    </button>
                </div>
            </div>
